  ...
  describe "update filter" do
    ...
    it "should successfully update the task filter" do
      WORD_LIST.each do |word|
        ...
        xhr :post, :update_current_filter, {:filter =>word, :redirect_action =>"/tasks/list", :task_filter =>{:unread_only=>"false", :qualifiers_attributes=>[{:qualifiable_id => id, :qualifiable_type => "TimeRange", :qualifiable_column =>"due_at", :reversed =>"false"}]}}
        response.should be_success
    ...
    it "should redirect to '/tasks/list' if an 'http' request" do
        WORD_LIST.each do |word|
          id = TimeRange.where(:name => word).first.id
          post :update_current_filter, {:filter =>word, :redirect_action =>"/tasks/list", :task_filter =>{:unread_only=>"false", :qualifiers_attributes=>[{:qualifiable_id => id, :qualifiable_type => "TimeRange", :qualifiable_column =>"due_at", :reversed =>"false"}]}}
