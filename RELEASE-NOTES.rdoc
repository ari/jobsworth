...
* IMPORTANT: Remove the project files feature. Files are now only linked to tasks. If you are looking for a broader file sharing system per project, we recommend DropBox, Google Files, or one of many other similar tools
* IMPORTANT: Remove the notes feature. This was never particularly helpful and cluttered the screen.
* Clean up tooltips
...
* Major refactor of the worklog/eventlog schema. No significant user visible changes except that some worklog are no longer duplicated.
