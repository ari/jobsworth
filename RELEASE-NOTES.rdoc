...
* email is now sent using a different (more robust) method. Delayed_job is no longer needed and anyone upgrading from 1.3 can kill the delayed_job process which might be running. Jobsworth 1.4 now starts up a different cron daemon to manage sending emails every 60 seconds.
