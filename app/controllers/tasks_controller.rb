...
#    if session[:filter_hidden].to_i == 0 || session[:filter_hidden].to_i == 2
#      filter << "tasks.hidden = 0 AND "
#    elsif session[:filter_hidden].to_i == 1
...
#    end
  ...
  def auto_complete_for_task_set_tags
    @tags = Tag.top_counts({ :like => params[:task][:set_tags].downcase, :company_id => session[:user].company_id, :project_ids => current_project_ids, :filter_user => session[:filter_user], :filter_hidden => session[:filter_hidden]})
  ...
  # Toggle the hidden status of a task
  def toggle_hidden
    task = Task.find(params[:id], :conditions => ["project_id = ?", session[:project].id])
    task.hidden = 1 - task.hidden;
    task.updated_by_id = session[:user].id
    task.save
    redirect_from_last
  end
  ...
  ## Toggle the done flag of a task
  def toggle_check
    @task = Task.find(params[:id], :conditions => ["project_id = ?", session[:project].id])
  ...
  def ajax_check
    ...
    if session[:history] && session[:history][0] == '/activities/list'
      ...
      @tasks = u.tasks.find(:all, :conditions => [" tasks.company_id = #{session[:user].company_id} AND tasks.project_id IN (#{current_project_ids}) AND tasks.completed_at IS NULL"],  :order => "tasks.severity_id + tasks.priority desc, CASE WHEN (tasks.due_at IS NULL AND milestones.due_at IS NULL) THEN 1 ELSE 0 END, CASE WHEN (tasks.due_at IS NULL AND tasks.milestone_id IS NOT NULL) THEN milestones.due_at ELSE tasks.due_at END LIMIT 5", :include => [:milestone]  )

      new_filter = ""
      new_filter = "AND tasks.id NOT IN (" + @tasks.collect{ |t| t.id}.join(', ') + ")" if @tasks.size > 0

      #FIXME: Show unassigned tasks
      @new_tasks = u.tasks.find(:all, :conditions => ["tasks.company_id = #{session[:user].company_id} AND tasks.project_id IN (#{current_project_ids}) #{new_filter} AND tasks.completed_at IS NULL"],  :order => "tasks.created_at desc", :include => [:milestone], :limit => 5  )
    end
  end
  ...
  def edit_log
    @log = WorkLog.find( params[:id], :conditions => ["company_id = ?", session[:user].company.id] )
    @log.started_at = tz.utc_to_local(@log.started_at)
    @task = @log.task
  ...
  def ajax_comments
    @comments = WorkLog.find(:all, :conditions => ["task_id = ? AND log_type = #{WorkLog::TASK_COMMENT}", params[:id]], :order => "started_at")
    render :partial => 'comment_list', :locals => { :comments => @comments }
  end

  def ajax_add_comment
    @task = Task.find(params[:id])
    @comment = WorkLog.new
    @comment.started_at = Time.now.utc
    @comment.body = params[:body]
    @comment.task = @task
    @comment.user = session[:user]
    @comment.company = session[:user].company
    @comment.project = @task.project
    @comment.customer = @task.project.customer
    @comment.duration = 0
    @comment.log_type = 2
    @comment.save
