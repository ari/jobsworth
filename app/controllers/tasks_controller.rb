...
#  end
  ...
  def ajax_check
    ...
    if session[:history] && session[:history][0] == '/activities/list'
      ...
      @new_tasks = u.tasks.find(:all, :conditions => ["tasks.company_id = #{session[:user].company_id} AND tasks.project_id IN (#{current_project_ids}) #{new_filter} AND tasks.completed_at IS NULL AND tasks.milestone_id NOT IN (#{completed_milestone_ids})"],  :order => "tasks.created_at desc", :include => [:milestone], :limit => 5  )
    end
  end
