...
#    :cancel_work_ajax, :destroy_log ]
  ...
  def list
    ...
    if params[:tag] && params[:tag].length > 0
      ...
      @selected_tags = []
      @tasks = Task.find(:all, :conditions => ["tasks.company_id = #{current_user.company_id} AND tasks.project_id IN (#{project_ids}) AND " + filter],  :order => " tasks.completed_at IS NOT NULL, tasks.completed_at desc, #{sort}", :include => [:users, :tags, :sheets, :todos, :dependencies, :dependants, { :project => :customer}, :milestone ])
    end
